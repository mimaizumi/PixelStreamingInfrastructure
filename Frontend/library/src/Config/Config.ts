...
export class Flags {
    ...
    static Project = 'Project' as const;
}
    ...
    private populateDefaultSettings(useUrlParams: boolean, settings?: Partial<AllSettings>): void {
        ...
        this.textParameters.set(
            ...
            new SettingText(
                TextParameters.RoomDesignId,
                'Room Design UUID',
                'Room Design ID on community site',
                settings && Object.prototype.hasOwnProperty.call(settings, TextParameters.RoomDesignId)
                    ? settings[TextParameters.RoomDesignId]
                    : null,
                useUrlParams,
                (id: string) => {
                    if (!id) return;

                    Logger.Info(`Room Design ID: ${id}`);
                }
            )
        );

        this.textParameters.set(
            TextParameters.OneTimeToken,
            new SettingText(
                TextParameters.OneTimeToken,
                'One Time Token',
                'One Time Token generated by community site and for a specific room',
                settings && Object.prototype.hasOwnProperty.call(settings, TextParameters.OneTimeToken)
                    ? settings[TextParameters.OneTimeToken]
                    : null,
                useUrlParams,
                (token: string) => {
                    if (!token) return;

                    Logger.Info(`One Time Token: ${token}`);
                }
            )
        );
        ...
        this.flags.set(
            ...
            new SettingFlag(
                ...
                'Project',
                'Project flag',
                settings && Object.prototype.hasOwnProperty.call(settings, Flags.Project)
                    ? settings[Flags.Project]
                    : false,
                useUrlParams
            )
        );
